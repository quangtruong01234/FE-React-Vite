...
describe('effectiveUnitPrice', () => {
  it('uses the matched SKU price when the line carries a skuId', () => {
    const p = product({ id: 'prod_1', price: 1000, skus: [{ id: 7, price: 2500 } as never] });
    expect(effectiveUnitPrice({ skuId: 7 }, p)).toBe(2500);
  });

  it('falls back to the product price when no skuId is set', () => {
    expect(effectiveUnitPrice({ skuId: null }, product({ id: 'prod_1', price: 1000 }))).toBe(1000);
  });
  ...
  it('coerces decimal-string prices from the backend', () => {
    const p = product({ id: 'prod_1', price: '1500.00' as unknown as number });
...
describe('buildShippingFeeItems', () => {
  const map = new Map<string, ProductWithInventory>([
    ['prod_1', product({ id: 'prod_1', name: 'Mug', price: 1000, weight: 300 })],
    ['prod_2', product({ id: 'prod_2', name: 'Heavy', price: 2000, weight: null })],
  ]);

  it('maps name, quantity, price and weight per line', () => {
    expect(
      buildShippingFeeItems([{ productId: 'prod_1', skuId: null, quantity: 2 }], map),
  ...
  it('passes undefined weight for an unweighed (null) product so BE uses its default', () => {
    expect(buildShippingFeeItems([{ productId: 'prod_2', skuId: null, quantity: 1 }], map)[0].weight).toBeUndefined();
  });

  it('handles a product missing from the map (price 0, no name/weight)', () => {
    expect(
      buildShippingFeeItems([{ productId: 'prod_99', skuId: null, quantity: 1 }], map),
