...
- **No native browser dialogs — ever.** `window.confirm`, `window.alert`, `window.prompt` (and their bare `confirm()` / `alert()` / `prompt()` forms) are **banned in `src/`**. They render as the browser's own chrome ("localhost:5173 says"), ignore the design system, block the main thread, cannot show a pending or error state, and are auto-suppressed by Chrome DevTools MCP / Playwright — so any flow behind one becomes unverifiable. Use a modal instead, in this order:
  1. **Confirm / cancel question →** `<ConfirmDialog>` (`components/shared/ConfirmDialog.tsx`). Props: `open`, `title`, `description`, `confirmLabel`, `cancelLabel?`, `tone?: 'default' | 'danger'`, `isPending?`, `error?`, `onConfirm`, `onCancel`.
  2. **Anything richer (a form, a choice, a preview) →** look for an existing modal first (`ReportPostDialog`, `AddressFormModal`, `EditProfileModal`, `CreatePostModal`, `FollowListModal`) and reuse it if it fits.
  3. Only if neither fits, build a new modal on `@/components/ui/dialog` following the `ReportPostDialog.tsx` idiom — and if the same shape shows up twice, it belongs in `components/shared/`.
  - A one-way notice (`alert`) is not a dialog at all: use the page's existing toast/notice banner, or the `error` prop of the dialog that caused it.
  - Async flows that used to sit mid-`await` on a `confirm` split in two: the handler opens the modal and returns, and the modal's `onConfirm` resumes the save (see `CreateProductPage.submitProduct`, `VoucherConsole.persist`).
