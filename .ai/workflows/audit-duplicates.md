...
Scan `src/` for hooks/components/utils that already do what you're about to write.
...
### Step 2 — Search hooks (`src/hooks/{auth,data,query,ui}/` + `src/features/**/use*.ts`)

> `src/hooks/` is layered — a "does this exist?" search must cover all four subfolders,
> not just the top level. Feature-local hooks live next to their page.

```bash
# Find hooks by name
find src -name 'use*.ts' -o -name 'use*.tsx' | xargs grep -l '<keyword>'

# Find exported function/const names matching keyword
grep -rn 'export (function|const) \w*<Keyword>' src --include='*.ts' --include='*.tsx'
```
...
```bash
grep -rn 'export (function|const|default)' src --include='*.tsx' | grep -i '<keyword>'
```

### Step 4 — Search utilities (`src/lib/{format,query,realtime,auth,domain,http}/` + `src/api/`)

```bash
grep -rn 'export ' src/lib src/api | grep -i '<keyword>'
```
...
If no matches:
```
── Audit: usePrintInvoice ───────────────────────
✅ No matches found in src/.
   Safe to create features/order/usePrintInvoice.ts.
─────────────────────────────────────────────────
```

> Placement rule after a clean audit: shared across 2+ features → `src/hooks/<layer>/`;
> used by one feature only → that feature's folder. See `.ai/context/structure.md`.
