...
> Cập nhật: 2026-08-26 · Phạm vi: frontend social + e-commerce (ưu tiên e-commerce).
...
**Gates (chạy lại + verify 2026-08-28):** `npm run build` ✓ · `npm run lint` 0 problem ·
`npm run test:run` **873 test / 112 file**, all pass. Không đóng item nào khi 3 lệnh này chưa xanh.
...
- **GHN-MSG-01 / GHN-WARD-01 (SWEEP-0828)** — cần backend live + một địa chỉ trỏ vào ward GHN từ
  chối. Repro rẻ nhất, **không** tạo đơn (chết ở bước tính phí): sửa một địa chỉ sang Quận 8
  (`districtId=1450`) + một trong chín ward `20801`-`20803` / `20808`-`20813` rồi vào `/checkout`.
  Kỳ vọng: câu tiếng Việt "…không nhận giao tới phường/xã đã chọn…" nằm **dưới mục 1**, cột tóm
  tắt chỉ còn dòng trỏ lên mục 1, nút đặt hàng disabled, và **không** có chữ `GHN`/`GET`/`/api/`
  nào lọt ra UI.
