...
> Cập nhật: 2026-09-09 · Phạm vi: frontend social + e-commerce (ưu tiên e-commerce).
...
**Gates (chạy lại + verify 2026-09-09):** `npm run build` ✓ · `npm run lint` 0 problem ·
...
- **CHAT-ROOM-01 — BE xong nhưng CHƯA lên prod; FE **cố ý chưa dọn**.** `1ea9ed6` chỉ có trên
  branch `feat/chat-room-01-user-rooms`, **không** có trên `api` `origin/main` — đo lại 2026-09-09:
  `origin/main` = `d08244c`, `merge-base --is-ancestor 1ea9ed6 d08244c` **NO**, `git grep
  chatMessageRooms d08244c` không hit. Nên `joinAll()` (`chatPresenceSocket.ts`) + re-join theo
