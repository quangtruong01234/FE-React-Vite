...
> Rule source: `context/performance.md`. This command catches only what is grep-detectable. Real performance must be MEASURED — see "What this cannot catch" at the end.
...
> Do NOT recommend useMemo/useCallback/React.memo from this scan alone. Recommend them only after a Profiler trace shows a real re-render cost. Otherwise you violate the "don't optimize speculatively" rule in `context/performance.md`.
